        ...
        tmp_dict.update(
            {
                ...
                "flow_time": ind.fitness.values[1] if len(ind.fitness.values) > 1 else None
            })
...
def results_saving(pareto_front, path, parameters):
    """
    Save the GA optimization results to a JSON file.
    """
    
    pareto_results = [{"makespan": ind.fitness.values[0], "flow_time": ind.fitness.values[1]} for ind in pareto_front]
    ...
    file_path = os.path.join(path, "NSGA2_results.json")
